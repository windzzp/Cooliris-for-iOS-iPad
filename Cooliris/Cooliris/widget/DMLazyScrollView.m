...
- (void) initializeControl {
    ...
    NSLog(@"initializeControl, (%f, %f, %f, %f)", self.frame.origin.x,
          self.frame.origin.y, self.frame.size.width, self.frame.size.height);
    
    CGSize dimensions = [UIApplication currentSize];
    CGRect rect = CGRectMake(0, 0, dimensions.width, dimensions.height);
    
    if (_direction == DMLazyScrollViewDirectionHorizontal) {
        self.contentSize = CGSizeMake(rect.size.width*5.0f, rect.size.height);
    } else {
        self.contentSize = CGSizeMake(rect.size.width, rect.size.height*5.0f);
...
- (UIViewController *) loadControllerAtIndex:(NSInteger) index andPlaceAtIndex:(NSInteger) destIndex {
    ...
    if (_direction == DMLazyScrollViewDirectionHorizontal) {
        ...
        NSLog(@"index = %d, (%f, %f, %f, %f)", destIndex, viewController.view.frame.origin.x,
              viewController.view.frame.origin.y, viewController.view.frame.size.width, viewController.view.frame.size.height);
