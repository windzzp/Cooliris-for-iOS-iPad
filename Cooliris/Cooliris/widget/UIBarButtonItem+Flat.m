...
#import "UIImage+Flat.h"

@implementation UIBarButtonItem (Flat)

+ (void) configureFlatButtonsWithColor:(UIColor *)color
                      highlightedColor:(UIColor *)highlightedColor
                          cornerRadius:(CGFloat) cornerRadius
{
    [self configureFlatButtonsWithColor:color
                       highlightedColor:highlightedColor
                           cornerRadius:cornerRadius
                        whenContainedIn:[UINavigationBar class], [UINavigationController class], nil];
}
                       ...
                       whenContainedIn:(Class <UIAppearanceContainer>)containerClass, ...
{
    ...
    va_end(vl);
    [UIBarButtonItem configureItemOrProxy:appearance
                   forFlatButtonWithColor:color
                         highlightedColor:highlightedColor
                             cornerRadius:cornerRadius];
}
                       ...
                       whenContainedIn:(Class <UIAppearanceContainer>)containerClass, ... 
{
    ...
    [UIBarButtonItem configureItemOrProxy:appearance
                   forFlatButtonWithColor:color
                         highlightedColor:highlightedColor
                                withImage:image
                        hightlightedImage:highlightedImage
                             cornerRadius:cornerRadius];
}
...
- (void) configureFlatButtonWithColor:(UIColor *)color
                     highlightedColor:(UIColor *)highlightedColor
                         cornerRadius:(CGFloat)cornerRadius
{
    [UIBarButtonItem configureItemOrProxy:self
                   forFlatButtonWithColor:color
                         highlightedColor:highlightedColor
                             cornerRadius:cornerRadius];
}
...
{
    NSMutableDictionary *titleTextAttributes = [[self titleTextAttributesForState:UIControlStateNormal]
                                                mutableCopy];
    if (!titleTextAttributes) {
        titleTextAttributes = [NSMutableDictionary dictionary];
    }
    
    [titleTextAttributes setValue:[NSValue valueWithUIOffset:UIOffsetMake(0, 0)]
