...
// Add by LZT (201201106)--Begin
/**
 * Append the clause to source string.
 *
 * @param sourceString the source string.
 * @param keyword      the SQL clause keyword
 * @param clause       the SQL clause
 */
- (void) appendClause : (NSMutableString*) sourceString
                      : (NSString*) keyword
                      : (NSString*) clause;

/**
 * Append the columns to source string.
 *
 * @param sourceString  the source string.
 * @param columns       the columns array
 * @param nCount        the columns count
 */
- (void) appendColumns : (NSMutableString*) sourceString
                       : (NSString*__strong*) columns
                       : (int) nCount;
// Add by LZT (201201106)--End
@end
...
{
    return [self executeQueryWithDistinct:false withTable:table withColumns:columns withColumnCount:nColumnCount withSelection:selection withSelectionArgs:selectionArgs withGroupBy:groupBy withHaving:having withOrderBy:orderBy withLimit:limit];
}
...
- (FMResultSet *)executeQueryWithDistinct: (BOOL)distinct withTable: (NSString*)table
                        ...
                        withSelectionArgs:(NSArray*) selectionArgs withGroupBy:(NSString*)groupBy
                               withHaving:(NSString*)having withOrderBy:(NSString*)orderBy withLimit:(NSString*)limit
{
    NSString *strSQLString = [self buildQueryStringWithDistinct:distinct withTable:table withColumns:columns withColumnsCount:nColumnCount withWhere:selection withGroupBy:groupBy withHaving:having withOrderBy:orderBy withLimit:limit];
    
    return [self executeQuery:strSQLString withArgumentsInArray:selectionArgs];
}

- (NSString *) buildQueryStringWithDistinct:(BOOL)distinct withTable:(NSString*)tables
                                withColumns:(NSString*__strong*)columns
                           withColumnsCount:(int)nCount
                                  withWhere:(NSString*) where withGroupBy:(NSString*)groupBy
                                 withHaving:(NSString*) having withOrderBy:(NSString*)orderBy
                                  withLimit:(NSString*) limit
{
    if((groupBy == nil || [groupBy isEqualToString:@""])
       && !(having == nil || [having isEqualToString:@""]))
    {
        return nil;
    }
    
    NSMutableString *strSQLClause = [[NSMutableString alloc] init];
    
    [strSQLClause appendString:@"SELECT "];
    
    if (distinct)
    {
        [strSQLClause appendString:@"DISTINCT "];
    }
    
    if (columns != nil && nCount > 0)
    {
        [self appendColumns:strSQLClause :columns :nCount];
    }
    else
    {
        [strSQLClause appendString:@"* "];
    }
    [strSQLClause appendString:@" FROM "];
    [strSQLClause appendString:tables];
    
    if (nil != where) {
        [self appendClause:strSQLClause :@" WHERE " :where];
        
    }
    if (nil != groupBy) {
        [self appendClause:strSQLClause :@" GROUP BY " :groupBy];
    }
    if (nil != having) {
        [self appendClause:strSQLClause :@" HAVING " :having];
    }
    if (nil != orderBy) {
        [self appendClause:strSQLClause :@" ORDER BY " :orderBy];
    }
    if (nil != limit) {
        [self appendClause:strSQLClause :@" LIMIT " :limit];
    }
    
    return strSQLClause;
}

- (void) appendClause : (NSMutableString*) sourceString  : (NSString*) keyword : (NSString*) clause
{
    if(nil != sourceString && clause != nil && ![clause isEqualToString:@""])
    {
        [sourceString appendString:keyword];
        [sourceString appendString:clause];
    }
}

- (void) appendColumns : (NSMutableString*) sourceString : (NSString*__strong*) columns : (int) nCount
{
    if(sourceString != nil && columns != nil && nCount > 0)
    {
        for(int nIndex = 0; nIndex < nCount; ++nIndex)
        {
            NSString *column = columns[nIndex];
            if(column != nil)
            {
                if(nIndex > 0)
                {
                ...
                [sourceString appendString:column];
            }
        }
    }
}
// Add by LZT (20121226)--End
...
- (BOOL)insertWithOnConflictTableName: (NSString*)table withNULLColumnHack: (NSString*)nullColumnHack withValues: (NSDictionary*)contentValues withConflict: (conflictType_e)eConflictAlgorithm
{
    ...
    NSMutableString *strSQLClause = [[NSMutableString alloc] initWithCapacity:152];
    [strSQLClause appendString:@"INSERT"];
    [strSQLClause appendString:CONFLICT_VALUES[eConflictAlgorithm]];
    [strSQLClause appendString:@" INTO "];
    [strSQLClause appendString:table];
    
    NSMutableString *strValue = [[NSMutableString alloc] initWithCapacity:40];
    NSArray * arrValues = nil;
    if(nil != contentValues && [contentValues count] > 0)
    {
        arrValues = [contentValues allValues];
        [strSQLClause appendString:@"("];
        BOOL needSeparator = NO;
        for (NSString *contentKey in [contentValues allKeys])
        {
            if(needSeparator)
            {
                [strSQLClause appendString:@", "];
            ...
            needSeparator = YES;
            [strSQLClause appendString:contentKey];
            [strValue appendString:@"?"];
        }
    ...
    {
        [strSQLClause appendFormat:@"(%@)", nullColumnHack];
        [strValue appendString:@"NULL"];
    }
    
    [strSQLClause appendString:@" VALUES("];
    [strSQLClause appendString:strValue];
    [strSQLClause appendString:@");"];
    
    bResult = [self executeUpdate:strSQLClause withArgumentsInArray:arrValues];
...
- (BOOL) updateWithOnConflictTableName: (NSString*)table withValues: (NSDictionary*)contentValues withWhereClause: (NSString*)whereClause withWhereArgs:   (NSString*[])whereArgs withConflict: (conflictType_e)eConflictAlgorithm
{
    ...
    if(contentValues == nil || [contentValues count] == 0)
    {
        return bResult;
    }
    
    NSMutableString *strSQLClause = [[NSMutableString alloc] initWithCapacity:120];
    [strSQLClause appendString:@"UPDATE "];
    [strSQLClause appendString:CONFLICT_VALUES[eConflictAlgorithm]];
    [strSQLClause appendString:table];
    [strSQLClause appendString:@" SET "];
    
    NSArray * arrValues = [contentValues allValues];
    BOOL needSeparator = NO;
    for (NSString *contentKey in [contentValues allKeys])
    {
        if(needSeparator)
    ...
    if(nil != whereClause && ![whereClause isEqualToString:@""])
    {
        [strSQLClause appendString:@" WHERE "];
        [strSQLClause appendString:whereClause];
    }
